...
public class Library {
	...
	void checkNum(String[] bookNumberList,boolean isCheckIn){
		...
		//for the checkOut case, check if the input numbers are all in numberedListOfSearch or not
		...
		//for the checkIn case, check if the input numbers are all in numberedListOfSearch or not
		for (int i=0; i<bookNumberList.length; i++){
			int num = Integer.parseInt(bookNumberList[i].trim());
		...
		if (j == 0){
			for (int i=0; i<bookNumberList.length; i++){
				int number = Integer.parseInt(bookNumberList[i].trim());
				if (this.numberedListOfSearch.containsKey(number) && ! isCheckIn){
